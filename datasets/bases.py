...
class BaseDataset(object):
    ...
    def get_imagedata_info(self, data):
        pids, cams, tracks, domains = [], [], [], []

        for _, pid, camid, trackid, domainid in data:
            pids += [pid]
            cams += [camid]
            tracks += [trackid]
            domains += [domainid]
...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, index):
        img_path, pid, camid, trackid, domainid = self.dataset[index]
        img = read_image(img_path)

        if self.transform is not None:
            img = self.transform(img)

        return img, pid, camid, trackid, img_path, domainid
